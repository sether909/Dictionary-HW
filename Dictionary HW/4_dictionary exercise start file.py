...
def deal_cards(deck, number):
    ...
    if number > 52:
        print("The number of cards to dealt, cannot be greater than # of cards in the deck (52).")
        return
    ...
    for i in range(number):
        if not deck:  # Check if the deck is empty
            ...
            break
        card = deck.popitem()  # Deal the top card (last added card)
        value = card[1]  # Get the value of the card
        print(f"Dealt {card[0]}, which is worth {value} points.")
        hand_value += value
